...
class HistogramDisplay: UIView
{
    ...
    func histogramCalculation(imageRef: CGImage) -> (red: [UInt], green: [UInt], blue: [UInt])
    {
        ...
        let alphaPtr:UnsafeMutablePointer<vImagePixelCount>?  = UnsafeMutablePointer<vImagePixelCount>.allocate(capacity: 256)
        alphaPtr?.initialize(from: alpha, count: 256)
        let redPtr:UnsafeMutablePointer<vImagePixelCount>? = UnsafeMutablePointer<vImagePixelCount>.allocate(capacity: 256)
        redPtr?.initialize(from: red, count: 256)
        let greenPtr:UnsafeMutablePointer<vImagePixelCount>? = UnsafeMutablePointer<vImagePixelCount>.allocate(capacity: 256)
        greenPtr?.initialize(from: green, count: 256)
        let bluePtr:UnsafeMutablePointer<vImagePixelCount>? = UnsafeMutablePointer<vImagePixelCount>.allocate(capacity: 256)
        bluePtr?.initialize(from: blue, count: 256)
    ...
    func drawHistogram()
    {
        ...
        drawChannel(data: histogram.red, maximum: maximum, layer: redLayer)
        drawChannel(data: histogram.green, maximum: maximum, layer: greenLayer)
        drawChannel(data: histogram.blue, maximum: maximum, layer: blueLayer)
    }
